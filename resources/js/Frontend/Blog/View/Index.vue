        ...
        <div class="page-content with-sidebar">
            <div class="container">
                <div class="row gutter-lg">
                    <div class="col-lg-9">
                        <div class="posts">
                            <article class="post post-list mb-4" v-for="(item, idx) in blogList.data" :key="idx">
                                <figure class="post-media overlay-zoom">
                                   <router-link :to="'/blog_detail/'+item.id">
                                        <img src="/frontend/images/blog/list/1.jpg" width="355" height="250" alt="post" />
                                   </router-link>
                                </figure>
                                <div class="post-details">
                                    <div class="post-meta">
                                        on <a href="#" class="post-date">{{getDateFormat(item.created_at)}}</a>
                                        | <a href="#" class="post-comment"><span>2</span> Comments</a>
                                    </div>
                                    <h4 class="post-title"><router-link :to="'/blog_detail/'+item.id">{{item.title}}</router-link>
                                    </h4>
                                    <p class="post-content" v-html="item.content"></p>
                                    <router-link :to="'/blog_detail/'+item.id" class="btn btn-link btn-underline btn-primary">Read more<i class="d-icon-arrow-right"></i>
                                    </router-link>
                                </div>
                            </article>
                        </div>
...
export default {
    ...
    methods:{
        ...
        getBlogList(){
            axios.get("/api/blog")
            .then(respone=>{
                this.blogList = respone.data.success;
            })
        },
    },
    mounted(){
